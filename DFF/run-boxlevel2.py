...
from models.models3 import get_model
from models.box_encoding import get_box_decoding_fn, get_box_encoding_fn, \
                          get_encoding_len
from models.flownet3d import get_flowed_feature_box
...
t_features, t_sence_flow = tf.cond(
    ...
    lambda : get_flowed_feature_box(
        ...
        t_key_graph_idx,    # pc2_graph_point_idx
        t_is_training,
        n_sample=2
    )
)
...
with tf.Session(graph=graph,
    ...
    for frame_idx in tqdm(range(0, NUM_TEST_SAMPLE)):
        ...
        if frame_idx % KEY_FRAME_STEP == 0:
            ...
            key_frame_info["probs"] = results["probs"]
            key_frame_info["pred_box"] = results["pred_box"]
            key_frame_info["key_graph_idx"] = keypoint_indices_list[0]
        else:
            # non key frame
            feed_dict = {
                t_initial_vertex_features: input_v,
                ...
                t_key_graph_idx: key_frame_info["key_graph_idx"]
            }
            feed_dict.update(dict(zip(t_edges_list, edges_list)))
            feed_dict.update(
                dict(zip(t_keypoint_indices_list, keypoint_indices_list)))
        ...
        if frame_idx % KEY_FRAME_STEP == 0:
            ...
            pred_boxes = results['pred_box'].reshape((-1, 1, BOX_ENCODING_LEN))
            last_layer_points_xyz = np.tile(
                np.expand_dims(key_frame_info['key_graph_xyz'], axis=1), (1, NUM_CLASSES, 1))
            last_layer_points_xyz = last_layer_points_xyz.reshape((-1, 3))
            boxes_centers = last_layer_points_xyz
            decoded_boxes = box_decoding_fn(np.expand_dims(box_labels, axis=1),
                boxes_centers, pred_boxes, label_map)
        ...
        if box_indices.size != 0:
            ...
            if frame_idx % KEY_FRAME_STEP != 0:
                K = 2
                key_points = key_frame_info["key_pc_xyz"]
                box_points = decoded_boxes[:, 0:3]
                flow_k_to_nk = results['sence_flow']
                box_shift = []
                    
                for i in range(len(box_points)):
                    query_point = box_points[i].reshape(1,3)
                    distance_i = np.sum(np.square(query_point - key_points), axis=1)
                    top_k_idx = np.argpartition(distance_i, K)[:K]
                    top_k_flow = flow_k_to_nk[top_k_idx]
                    mean_flow = np.mean(top_k_flow, axis=0)
                    box_shift.append(mean_flow)
                box_shift = np.array(box_shift)
                print(box_shift)
                decoded_boxes[:, 0:3] = decoded_boxes[:, 0:3] + box_shift
            # nms ============================================================
            ...
            #     K = 2
            #     key_points = key_frame_info["key_pc_xyz"]
            #     box_points = detection_boxes_3d[:, 0:3]
            #     flow_k_to_nk = results['sence_flow']
            #     box_shift = []
            ...
            #     box_shift = np.array(box_shift)
            #     print(box_shift)
            #     detection_boxes_3d[:, 0:3] = detection_boxes_3d[:, 0:3] + box_shift
