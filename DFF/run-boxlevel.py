...
from models.box_encoding import get_box_decoding_fn, get_box_encoding_fn, \
                          get_encoding_len
from models.flownet3d import get_flowed_feature, get_box_level_feature
...
t_key_graph_logits = tf.placeholder(dtype=tf.float32, shape=[None, 4])
t_key_graph_pred_box = tf.placeholder(dtype=tf.float32, shape=[None, 4, 7])
# =======================

model = get_model(config['model_name'])(num_classes=NUM_CLASSES,
    box_encoding_len=BOX_ENCODING_LEN, mode='test', **config['model_kwargs'])

t_logits, t_pred_box = tf.cond(
    t_is_keyframe,
    ...
    lambda : get_box_level_feature(
        t_key_pc_xyz,
        t_key_pc_rgb,
        t_key_graph_xyz,
        t_key_graph_logits,
        t_key_graph_pred_box,
...
with tf.Session(graph=graph,
    ...
    key_frame_info = {
        "key_pc_xyz": None,
        "key_pc_rgb": None,
        "key_graph_logits": None,
        "key_graph_pred_box": None,
        "key_graph_xyz": None
    }
    KEY_FRAME_STEP = 2 
    for frame_idx in tqdm(range(0, NUM_TEST_SAMPLE)):
        ...
        if frame_idx % KEY_FRAME_STEP == 0:
            ...
            feed_dict = {
                ...
                t_key_graph_logits: np.zeros([10, 4]),
                t_key_graph_pred_box: np.zeros([10,4,7])
            }
            ...
            key_frame_info["key_graph_logits"] = results["logits"]
            key_frame_info["key_graph_pred_box"] = results["pred_box"]
            ...
            feed_dict = {
                ...
                t_key_graph_logits: key_frame_info["key_graph_logits"],
                t_key_graph_pred_box: key_frame_info["key_graph_pred_box"]
            }
            feed_dict.update(dict(zip(t_edges_list, edges_list)))
            feed_dict.update(
                dict(zip(t_keypoint_indices_list, keypoint_indices_list)))
        ...
        # print("results['probs'].shape:", results['probs'].shape)
        # print("results['pred_box'].shape:", results['pred_box'].shape)
