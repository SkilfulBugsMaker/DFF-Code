...
from models.flownet3d import get_flownet3d_small_model, get_flownet3d_large_model, feature_flow, get_flowed_feature_train
...
for gi in range(NUM_GPU):
    with tf.device('/gpu:%d'%gi):
        for cp_idx in range(COPY_PER_GPU):
            # begin placeholder key_frame
            # begin initial vertex features placeholder, dim=[batch_size, k]
            if config['input_features'] == 'irgb':
                ...
                t_initial_vertex_features = tf.placeholder(
                    dtype=tf.float32, shape=[None, 1])
            # end begin initial vertex features placeholder
            ...
            # end placeholder key_frame

            # ====================== modify below ========================
            # begin placeholder non_key_frame
            if config['input_features'] == 'irgb':
                t_initial_vertex_features1 = tf.placeholder(
                    dtype=tf.float32, shape=[None, 4])
            elif config['input_features'] == 'rgb':
                t_initial_vertex_features1 = tf.placeholder(
                    dtype=tf.float32, shape=[None, 3])
            ...
            t_vertex_coord_list1 = [
                tf.placeholder(dtype=tf.float32, shape=[None, 3])]
            for _ in range(len(config['graph_gen_kwargs']['level_configs'])):
                t_vertex_coord_list1.append(
                    tf.placeholder(dtype=tf.float32, shape=[None, 3]))

            t_edges_list1 = []
            for _ in range(len(config['graph_gen_kwargs']['level_configs'])):
                t_edges_list1.append(
                    tf.placeholder(dtype=tf.int32, shape=[None, None]))

            t_keypoint_indices_list1 = []
            for _ in range(len(config['graph_gen_kwargs']['level_configs'])):
                t_keypoint_indices_list1.append(
                    tf.placeholder(dtype=tf.int32, shape=[None, 1]))

            t_class_labels1 = tf.placeholder(dtype=tf.int32, shape=[None, 1])
            t_encoded_gt_boxes1 = tf.placeholder(
                dtype=tf.float32, shape=[None, 1, BOX_ENCODING_LEN])
            t_valid_gt_boxes1 = tf.placeholder(
                dtype=tf.float32, shape=[None, 1, 1])
            # end placeholder non_key_frame
            # ====================== modify above ========================
            ...
            t_features = model.extract_features(
                ...
                t_keypoint_indices_list, t_edges_list, tf.constant(False))
            # ======================= modify below =======================
            # predict non-key frame to key frame optical flow, 几万点

            t_features1 = get_flowed_feature_train(
                t_vertex_coord_list[0],
                t_initial_vertex_features,
                t_vertex_coord_list[1],
                t_features,
                t_vertex_coord_list1[0],     # pc2_xyz
                t_initial_vertex_features1,  # pc2_rgb
                t_vertex_coord_list1[1],    # pc2_graph_point_xyz
                t_keypoint_indices_list1[0],    # pc2_graph_point_idx
                t_is_training,
                n_sample=1 
            )
            ...
            t_logits, t_pred_box = model.predict(t_features1, tf.constant(False))
            t_probs = model.postprocess(t_logits)
            t_predictions = tf.argmax(t_probs, axis=-1, output_type=tf.int32)
            t_loss_dict = model.loss(t_logits, t_class_labels1, t_pred_box,
                t_encoded_gt_boxes1, t_valid_gt_boxes1, **config['loss'])
            ...
            # ==================== modify below ===================
            # t_logits1, t_pred_box1 = model.predict(t_features1, t_is_training)
            # t_probs1 = model.postprocess(t_logits1)
            # t_predictions1 = tf.argmax(t_probs1, axis=-1, output_type=tf.int32)
            # t_loss_dict1 = model.loss(
            #     t_logits1, t_class_labels1, t_pred_box1,
            #     t_encoded_gt_boxes1, t_valid_gt_boxes1, **config['loss']
            # )
            # t_cls_loss1 = t_loss_dict1['cls_loss']
            # t_loc_loss1 = t_loss_dict1['loc_loss']
            # t_reg_loss1 = t_loss_dict1['reg_loss']
            # t_num_endpoint1 = t_loss_dict1['num_endpoint']
            ...
            input_tensor_sets.append(
                {
                    ...
                    't_initial_vertex_features1': t_initial_vertex_features1,
                    't_vertex_coord_list1': t_vertex_coord_list1,
                    't_edges_list1': t_edges_list1,
                    't_keypoint_indices_list1': t_keypoint_indices_list1,
                    't_class_labels1': t_class_labels1,
                    't_encoded_gt_boxes1': t_encoded_gt_boxes1,
                    't_valid_gt_boxes1': t_valid_gt_boxes1,
                     # ================== modify ===================

                    't_is_training': t_is_training,

                    't_features': t_features,
                    't_features1': t_features1,
                    ...
                    # 't_logits1': t_logits1,
                    # 't_pred_box1': t_pred_box1,
                    # 't_probs1': t_probs1,
                    # 't_predictions1': t_predictions1,
                    # 't_cls_loss1': t_cls_loss1,
                    # 't_loc_loss1': t_loc_loss1,
                    # 't_reg_loss1': t_reg_loss1,
...
t_class_labels = input_tensor_sets[0]['t_class_labels1']
        ...
        for batch_idx in range(0, NUM_TEST_SAMPLE // 2, batch_size):
            ...
            for gi in range(COPY_PER_GPU*NUM_GPU):
                ...
                feed_dict = {
                    ...
                    t_initial_vertex_features1: input_v1,
                    t_class_labels1: cls_labels1,
                    t_encoded_gt_boxes1: encoded_boxes1,
                    t_valid_gt_boxes1: valid_boxes1,
                ...
                feed_dict.update(dict(zip(t_edges_list1, edges_list1)))
                feed_dict.update(
                    dict(zip(t_keypoint_indices_list1, keypoint_indices_list1)))
                feed_dict.update(
                    dict(zip(t_vertex_coord_list1, vertex_coord_list1)))
