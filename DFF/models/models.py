...
class MultiLayerFastLocalGraphModelV2(object):
    ...
    def predict(self, tfeatures, is_training):
        with slim.arg_scope([slim.batch_norm], is_training=is_training), slim.arg_scope([slim.fully_connected],
            ...
            with tf.variable_scope(predictor_config['scope'], reuse=tf.AUTO_REUSE):
                logits, box_encodings = predictor.apply_regular(tfeatures, num_classes=self.num_classes,
                                                                box_encoding_len=self.box_encoding_len, **predictor_config['kwargs'])
