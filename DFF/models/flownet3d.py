...
def get_box_level_feature(
    pc1_xyz, pc1_rgb, pc1_graph_point_xyz, pc1_logit, pc1_box_encode,
    ...
    optical_flow_non_key_to_key = get_flownet3d_large_model(
        pc_non_key_xyz,
        # pc_non_key_feat,
        tf.zeros_like(pc_non_key_xyz),
        pc_key_xyz,
        # pc_key_feat,
        tf.zeros_like(pc_key_xyz),
    ...
    pc2_logit = feature_flow(
        xyz,
        xyz1,
        optical_flow_non_key_to_key_graph_points,
        tf.expand_dims(pc1_logit, 0),
        n_sample=n_sample
    )
    pc2_box_encode = feature_flow(
        xyz,
        xyz1,
        optical_flow_non_key_to_key_graph_points,
        tf.expand_dims(pc1_box_encode, 0),
        n_sample=n_sample
    )
    pc2_logit = tf.squeeze(pc2_logit, axis=0)
    pc2_box_encode = tf.squeeze(pc2_box_encode, axis=0)
    return pc2_logit, pc2_box_encode


def get_flowed_feature(
        pc1_xyz, pc1_rgb, pc1_graph_point_xyz, pc1_graph_feat,
        pc2_xyz, pc2_rgb, pc2_graph_point_xyz, pc2_graph_point_idx,
        t_is_training, n_sample=4, n_sample2=8, agg=True
):
    # 1 key, 2 non key
    pc_non_key_xyz = tf.expand_dims(pc2_xyz, 0)
    pc_non_key_feat = tf.expand_dims(pc2_rgb, 0)
    ...
    if agg:
        _, idx = knn_point(n_sample2, pc_non_key_xyz, xyz1)
        neighbour_flow = tf.gather(optical_flow_non_key_to_key, idx[0], axis=1)
        optical_flow_non_key_to_key_graph_points = tf.reduce_mean(neighbour_flow, axis=2)
        # optical_flow_non_key_to_key_graph_points = tf.gather(
        #     optical_flow_non_key_to_key, pc2_graph_point_idx, axis=1
        # )
        # optical_flow_non_key_to_key_graph_points = tf.squeeze(
        #     optical_flow_non_key_to_key_graph_points, axis=2
        # )
        # optical_flow_non_key_to_key_graph_points = (optical_flow_non_key_to_key_graph_points + neighbour_flow_mean) / 2
    else:
        optical_flow_non_key_to_key_graph_points = tf.gather(
            optical_flow_non_key_to_key, pc2_graph_point_idx, axis=1
        )
...
def get_flowed_feature_train(
        pc1_xyz, pc1_rgb, pc1_graph_point_xyz, pc1_graph_feat,
        pc2_xyz, pc2_rgb, pc2_graph_point_xyz, pc2_graph_point_idx,
        t_is_training, n_sample=1
):
    # 1 key, 2 non key
    pc_non_key_xyz = tf.expand_dims(pc2_xyz, 0)
    pc_non_key_feat = tf.expand_dims(pc2_rgb, 0)
...
def get_flowed_feature_box(
    ...
    optical_flow_key_to_non_key = get_flownet3d_large_model(
        pc_key_xyz,
        # pc_non_key_feat,
        tf.zeros_like(pc_key_xyz),
        pc_non_key_xyz,
        ...
        tf.zeros_like(pc_non_key_xyz),
        is_training=t_is_training)
    print(optical_flow_key_to_non_key)

    sence_flow = tf.squeeze(optical_flow_key_to_non_key, 0)

    
    return pc1_graph_feat, sence_flow
...
def get_sence_flow(
        pc1_xyz, pc1_rgb, 
        pc2_xyz, pc2_rgb, pc2_graph_point_idx,
        t_is_training
):
    # 1 key, 2 non key
    pc_non_key_xyz = tf.expand_dims(pc2_xyz, 0)
    ...
    # [N_non_key_graph, 3]
    sence_flow = tf.squeeze(optical_flow_non_key_to_key_graph_points, axis=0)
...
def get_flownet3d_huge_model(pc1_xyz, pc1_feat, pc2_xyz, pc2_feat, is_training, bn_decay=None, reuse=False):
    # Modify
    """ FlowNet3D
        input:
            batch_size = 1
    ...
    with tf.variable_scope('sa1') as scope:
        ...
        # Frame 1, Layer 1
        l1_xyz_f1, l1_points_f1, l1_indices_f1 = pointnet_sa_module(l0_xyz_f1, l0_points_f1, npoint=16384, radius=RADIUS1, nsample=256, mlp=[32,32,64], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='layer1')
        end_points['l1_indices_f1'] = l1_indices_f1

        # Frame 1, Layer 2
        l2_xyz_f1, l2_points_f1, l2_indices_f1 = pointnet_sa_module(l1_xyz_f1, l1_points_f1, npoint=8192, radius=RADIUS2, nsample=256, mlp=[64,64,128], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='layer2')
        end_points['l2_indices_f1'] = l2_indices_f1
        ...
        # Frame 2, Layer 1
        l1_xyz_f2, l1_points_f2, l1_indices_f2 = pointnet_sa_module(l0_xyz_f2, l0_points_f2, npoint=16384, radius=RADIUS1, nsample=256, mlp=[32,32,64], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='layer1')
        # Frame 2, Layer 2
        l2_xyz_f2, l2_points_f2, l2_indices_f2 = pointnet_sa_module(l1_xyz_f2, l1_points_f2, npoint=8192, radius=RADIUS2, nsample=256, mlp=[64,64,128], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='layer2')

    _, l2_points_f1_new = flow_embedding_module(l2_xyz_f1, l2_xyz_f2, l2_points_f1, l2_points_f2, radius=3.0, nsample=256, mlp=[128,128,128], is_training=is_training, bn_decay=bn_decay, scope='flow_embedding', bn=True, pooling='max', knn=True, corr_func='concat')

    # Layer 3
    l3_xyz_f1, l3_points_f1, l3_indices_f1 = pointnet_sa_module(l2_xyz_f1, l2_points_f1_new, npoint=2048, radius=RADIUS3, nsample=64, mlp=[128,128,256], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='layer3')
    end_points['l3_indices_f1'] = l3_indices_f1

    # Layer 4
    l4_xyz_f1, l4_points_f1, l4_indices_f1 = pointnet_sa_module(l3_xyz_f1, l3_points_f1, npoint=512, radius=RADIUS4, nsample=64, mlp=[256,256,512], mlp2=None, group_all=False, is_training=is_training, bn_decay=bn_decay, scope='layer4')
    end_points['l4_indices_f1'] = l4_indices_f1
